...
namespace HzPrint
{
    internal class Ssd1681_154D67 : Ssd168x
    {
        ...
        {
            SendCommand(18);
            WaitReady(waitsec, new CancellationTokenSource(1));
        }

        public bool WaitReady(CancellationToken cancellationToken = default)
            => WaitUntilPinValueEquals(PinValue.Low, cancellationToken);

        public bool WaitUntilPinValueEquals(
        PinValue desiredValue,
        CancellationToken cancellationToken = default)
        {
            while (!cancellationToken.IsCancellationRequested && _pioController.Read(_busyPin) != desiredValue)
            {
